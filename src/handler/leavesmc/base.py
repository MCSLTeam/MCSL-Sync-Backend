...
class LeavesLoader(object):
    ...
    async def load_self(self, retry: int = 0) -> None:
        ...
        tmp_data = await get_json(
            "https://api.leavesmc.org/v2/projects/{project_id}/".format(
...
class SingleVersion(object):
    ...
    async def load_self(self, retry: int = 0) -> None:
        ...
        tmp_data = await get_json(
            "https://api.leavesmc.org/v2/projects/{project_id}/versions/{version}/".format(
...
class SingleDownload(object):
    def __init__(self, data: dict, name: str, version: str, build: int) -> None:
        ...
        self.link: str = "https://api.leavesmc.org/v2/projects/{name}/versions/{version}/builds/{build}/downloads/{file_name}/".format(
...
class BuildsManager(object):
    ...
    async def load_self(self) -> None:
        try:
            tmp_data = await get_json(
                "https://api.leavesmc.org/v2/projects/{project_id}/versions/{version}/builds/".format(
                    project_id=self.project_id, version=self.version
                )
            )
            tmp_build_info_list = [
                await get_json(
                    "https://api.leavesmc.org/v2/projects/{name}/versions/{version}/builds/{build}/".format(
